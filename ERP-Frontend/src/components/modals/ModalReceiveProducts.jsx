...
export default function ModalReceiveProducts({ title, saveButtonText, onClose }) {
  ...
  const searchForProducts = async () => {
    ...
    const baseUrl = `https://localhost:7011/Product?businessId=${currentBusiness}&skip=0&limit=100&search=${search}`;
  ...
  const submitAll = async () => {
    ...
    const res = await fetch("https://localhost:7011/Shipment/Receive", {
      method: "POST",
    ...
    if (!res.ok) {
      console.error(`Erro ao adicionar items`, res.status);
      if (res.status === 404) setUser(null);
    }
            ...
            {inventories.map((inv) => (
              <div
                ...
                className={`border rounded p-2 cursor-pointer ${
                  selectedInventory?.id === inv.id ? "bg-blue-100 border-blue-300" : "bg-white"
                } hover:bg-blue-50`}
